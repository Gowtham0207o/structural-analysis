...
if(isset($_POST['logout'])){
  ...
  function reaction_calc($data){
    ...
    // Iterate through the JSON-decoded data and store support locations and types
    foreach ($data as $element) {
        if (isset($element['type']) && (($element['type'] === "Support-pinned") || ($element['type'] === "Support-fixed") || ($element['type'] === "Support-roller"))) {
            $loc_parts = explode("|", $element['loc']);
            $length = intval(trim($loc_parts[1]));
            $supports[] = ['x' => $length, 'type' => $element['type']];
        }
    }

    // Calculate reaction forces for each support
    $output = [];
    foreach ($supports as $support) {
        $reaction = calculate_reaction($data, $support['x']);
        $output[] = ['x' => $support['x'], 'y' => $reaction, 'type' => $support['type']];
    }

    return json_encode($output);
}

// Function to calculate reaction at a given support location
function calculate_reaction($data, $support_loc) {
    $va = 0;
    $total = 0;
    $total_length = $support_loc;
    ...
    foreach ($data as $lelement) { // for distributed load calculation
        if (isset($lelement['type']) && ($lelement['type'] === "Dt. Load")) {
            $loc_parts = explode("|", $lelement['loc']);
            $load_parts = explode("|", $lelement['load']);

            $length_start = intval(trim($loc_parts[0]));
            $length_end = intval(trim($loc_parts[1]));
            $load_start = intval(trim($load_parts[0]));
            $load_end = intval(trim($load_parts[1]));

            $length = ($length_start + $length_end) / 2;
            $load = ($load_start + $load_end) / 2;
    ...
    return $total - $va; // Return reaction force
}
